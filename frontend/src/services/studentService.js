...
import { studentProfile, curricularFacets, prerequisiteBlocker } from '../data/mockData';
...
export const studentService = {
  ...
  async getMastery(student_id) {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (!error && data && data.length > 0) {
          const overall = Math.round((data.reduce((acc, curr) => acc + (curr.score || 0), 0) / data.length) * 100);
          return {
            ok: true,
            data: {
              student_id,
    ...
    // 3. Structured fallback using student defaults
    return {
      ok: true,
      data: {
        student_id,
        overall_score: (studentProfile.overallMastery || 68) / 100,
        subjects: curricularFacets.map(f => ({
          subject: f.id,
          average_score: f.mastery / 100,
          topic_count: f.conceptCount,
          weak_topics: f.status === 'WEAK' ? [f.criticalConcept] : [],
        })),
        topics: [
          { subject: 'maths', topic: 'conditional_probability', score: 0.42, state: 'weak', attempts: 3, source: 'bkt' },
          { subject: 'maths', topic: 'bayes_theorem', score: 0.68, state: 'learning', attempts: 2, source: 'bkt' },
          { subject: 'aiml', topic: 'naive_bayes_classifier', score: 0.67, state: 'learning', attempts: 2, source: 'bkt' },
          { subject: 'dsa', topic: 'graph_traversals', score: 0.74, state: 'learning', attempts: 4, source: 'bkt' },
          { subject: 'dbms', topic: 'sql_indexing', score: 0.92, state: 'mastered', attempts: 5, source: 'bkt' },
        ],
      },
      isDefault: true,
    };
        ...
        entries: [
          {
            ...
            agent: 'maths',
            confidence: 0.95,
            message: 'Explain Bayes Theorem formula derivation and conditional independence',
          ...
          },
          {
            ...
            agent: 'dsa',
            confidence: 0.92,
            message: 'Dynamic Programming subproblem recurrence for 0/1 Knapsack',
            routed_reason: 'Algorithmic complexity analysis dispatched to DSA Agent',
        ...
        recommendations: [
          {
            ...
            reason: 'Prerequisite gap detected prior to Naive Bayes optimization',
            priority: 'high',
            done: false,
          },
          {
  ...
  async submitAssessment(payload) {
    ...
    return {
      ...
      data: {
        ...
        misconception_logged: payload.is_correct ? null : (payload.misconception_type || 'Prior ignorance'),
...
function formatSubjectsFromTopics(topics) {
  ...
  return Object.keys(groups).map((subj) => {
    ...
    return {
      ...
      weak_topics: items.filter((i) => i.state === 'weak' || i.score < 0.5).map((i) => i.topic),
    };
  });
}
