...
export default function CurricularFacetCard({ facet }) {
  ...
  return (
    <div
      className={`bg-white rounded-xl p-5 border shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex flex-col justify-between relative overflow-hidden transition-all duration-150 hover:border-[#D4CCBE] ${
        facet.hasLeftAccent || isWeak
          ? "border-l-4 border-l-[#B93826] border-[#EAE5DC]"
          : "border-[#EAE5DC]"
      }`}
    >
      {/* Card Header */}
      <div>
        ...
        <div className="w-full bg-[#EAE5DC] h-1.5 rounded-full mt-3.5 overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.min(100, Math.max(0, facet.mastery ?? 0))}%`,
            }}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-[#F0ECE1]">
        ...
        >
          ...
          >
            {topStrength.badge ||
              topStrength.detail ||
              `${topStrength.score ?? facet.mastery ?? 0}%`}
          </div>
        </div>
