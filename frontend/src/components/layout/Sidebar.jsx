...
export default function Sidebar({ isOpen, onClose }) {
  ...
  // Format track name only for display.
  ...
  const displayTrack = formatTrackName(profile?.track || displayGoal);

  const displayMastery = profile?.mastery ?? 0;
    ...
    <>
      ...
      <aside
        className={`
          ...
          bg-[#161514] text-[#D1CCC5]
          flex flex-col
          border-r border-[#2C2926]
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 shrink-0 transition-transform ${
                        isActive
                          ? "text-white"
                          : "text-[#8E8880] group-hover:text-white group-hover:scale-105"
                      }`}
                    />
