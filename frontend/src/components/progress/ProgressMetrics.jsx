...
export default function ProgressMetrics({ progressData }) {
  ...
  return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <Metric title="Practice Volume" value={focused.attempts ?? 0} footer="Assessment attempts recorded" side={focused.attempts ? 'Live' : 'No data'} />
    <Metric title="Active Domains" icon={<Clock className="w-3.5 h-3.5 text-[#C07D1C]" />} value={focused.activeDomains ?? 0} badge={focused.attempts ? 'Tracked' : 'No activity'} footer={focused.scope || 'No active domains'} />
